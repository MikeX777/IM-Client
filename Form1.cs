...
namespace InstantMessenger
{
    public partial class Form1 : Form
    {
        ...
        public Dictionary<string, TalkForm> tfs { set; get; }
        IMClient im;
        public Form1()
        {
            InitializeComponent();
        ...
        void im_Disconnected(object sender, EventArgs e)
        {
            this.BeginInvoke(new MethodInvoker(delegate
            {
                ...
                foreach (string tf in talks.Keys)
                {
                    talks[tf].Close();
                }
            }));
        ...
        public Dictionary<string, TalkForm> talks = new Dictionary<string, TalkForm>();
        private void talkButton_Click(object sender, EventArgs e)
        {
            TalkForm tf = new TalkForm(im, sendTo.Text);
            talks.Add(sendTo.Text, tf);
            sendTo.Text = "";            
            tf.Show();
        }
